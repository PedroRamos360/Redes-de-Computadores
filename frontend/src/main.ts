...
api.get("/sniffer-reports").then((data) => {
  const sourceIpGraph = new BarGraph(
    ...
    "Pacotes gerado por IP de origem"
  );
  sourceIpGraph.render();
  const destinationIpGraph = new BarGraph(
    data.destinationIps,
    "visits-per-destination",
    "Pacotes recebidos por IP de destino"
  );
  destinationIpGraph.render();
  const destinationCountriesGraph = new BarGraph(
    data.destinationCountries,
    "visits-per-country",
    "Pacotes recebidos por país"
  );
